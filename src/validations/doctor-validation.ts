...
export const updateDoctorValidation = z
  .object({
    name: z.string({required_error: "Nama harus diisi"}).min(1, "Nama harus diisi").optional(),
    gender: z.nativeEnum(Gender, {required_error: "Jenis kelamin harus diisi"}).optional(),
    birthDate: z.coerce.date().optional(),
    birthPlace: z.string({required_error: "Tempat lahir harus diisi"}).min(1, "Tempat lahir harus diisi").optional(),
    nip: z.string({required_error: "NIP harus diisi"}).min(1, "NIP harus diisi"),
    nik: z.string().length(16, "NIK harus 16 digit").optional(),
    sip: z.string({required_error: "SIP harus diisi"}).min(1, "SIP harus diisi").optional(),
    str: z.string({required_error: "STR harus diisi"}).min(1, "STR harus diisi").optional(),
    specialization: z.string({required_error: "Spesialisasi harus diisi"}).min(1, "Spesialisasi harus diisi").optional(),
    status: z.nativeEnum(DoctorStatus, {required_error: "Status harus diisi"}).optional(),
    address: z
      .object({
        use: z
          .string()
          .min(1, "bisa di isi dengan nilai rumah/kantor/lainnya")
          .optional(),
        line: z.string({required_error: "Alamat harus diisi"}).min(1, "Alamat harus diisi").optional(),
        city: z.string({required_error: "Kota harus diisi"}).min(1, "Kota harus diisi").optional(),
        postalCode: z.string({required_error: "Kode pos harus diisi"}).min(1, "Kode pos harus diisi").optional(),
        country: z.string({required_error: "Negara harus diisi"}).min(1, "Negara harus diisi").optional(),
        extension: z
          .object({
            provinceCode: z
