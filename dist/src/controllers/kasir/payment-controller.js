...
exports.confirmPayment = exports.createPayment = exports.updatePaymentStatus = exports.handleMidtransNotification = exports.createPaymentWithMidtrans = exports.getPaymentByRegistrationId = void 0;
...
const getPaymentByRegistrationId = async (req, res, next) => {
    try {
        ...
        const medicineCost = encounter.prescriptions.reduce((total, prescription) => {
        ...
        const procedureCost = encounter.procedure.reduce((total, proc) => {
            const tindakan = encounter.patientRegistration.room.Tindakan.find((t) => t.icdCode === proc.icd9.code);
            return total + (tindakan?.finalTarif || 0);
        }, 0);
        // Biaya konsultasi dokter
        const consultationCost = encounter.patientRegistration.doctor.consultationFee || 0;
        ...
        // Total keseluruhan
        const totalAmount = medicineCost + procedureCost + consultationCost + servicesClass;
        const paymentData = {
            totalAmount,
        ...
        res.status(200).json({
            ...
            data: {
                ...
                patientInfo: {
                    ...
                    medicalRecordNumber: encounter.patientRegistration.patient.medicalRecordNumber,
                },
                doctorInfo: {
...
exports.getPaymentByRegistrationId = getPaymentByRegistrationId;
const createPaymentWithMidtrans = async (req, res, next) => {
    try {
        // Validasi Data
        const validatedData = payment_validation_1.createPaymentSchema.parse(req.body);
        const { patientRegistrationId, first_name, last_name, email, phone } = validatedData;
        const snap = (0, midtrans_1.createMidtransClient)();
        const encounter = await prisma.encounter.findUnique({
            where: {
        ...
        const payment = await prisma.payment.create({
            data: {
                paymentNumber: await (0, generate_1.generatePaymentNumber)(),
...
exports.createPaymentWithMidtrans = createPaymentWithMidtrans;
...
const createPayment = async (req, res, next) => {
    try {
        ...
        // Hitung biaya-biaya
        const medicineCost = encounter.prescriptions.reduce((total, prescription) => total + prescription.medicine.price * prescription.quantity, 0);
        const procedureCost = encounter.procedure.reduce((total, proc) => {
            const tindakan = encounter.patientRegistration.room.Tindakan.find((t) => t.icdCode === proc.icd9.code);
            return total + (tindakan?.finalTarif || 0);
        }, 0);
